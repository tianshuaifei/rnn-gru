...
def generate_sentence(model, x_train):
    new_sentence1=['result']
    next_word_probs1 = model.predict(x_train)[-1]
    samples1 = np.random.multinomial(1, next_word_probs1)
    sampled_word1 = np.argmax(samples1)
    new_sentence1.append(sampled_word1)
